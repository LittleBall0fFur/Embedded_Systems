...
class SoundSensor{
  public:
    SoundSensor(int _tempo, int _buzzer){
      this->_tempo = _tempo;
      this->_buzzer = _buzzer;
       play(false);
    };
 
    void play(bool _repeat= false){
      int notes = (sizeof(melody) / sizeof(melody[0])) / 2;
      int wholenote = (60000 * 4) / _tempo;
      data = MelodyData { _buzzer, notes, wholenote };
      threads.addThread(SoundSensor::thread_play);
    };
    ...
    static void thread_play(){
      ...
      for (int thisNote = 0; thisNote < arraySize; thisNote = thisNote + 2) {
          _data->divider = melody[thisNote + 1];
          if (_data->divider > 0) {
          ...
          tone(21, melody[thisNote], _data->noteDuration * 0.9);
